...
def wishMe():
    ...
    if(hour>=0) and (hour<=12) and ('AM' in t):
        speak(f"Good morning Farhan, it's {day} and the time is {t}")
    elif(hour>=12)  and (hour<=16) and ('PM' in t):
        speak(f"Good afternoon Farhan, it's {day} and the time is {t}")
    else:
        speak(f"Good evening Farhan, it's {day} and the time is {t}")
